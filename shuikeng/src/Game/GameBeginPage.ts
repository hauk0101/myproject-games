...
class GameBeginPage extends eui.Component{
    ...
    private startBtn:egret.MovieClip;
    ...
    private TextTimerFunc():void{
        ...
        if(this.textFlg == this.texts.length-1 &&(!this.hasStartBtn))
        {
            ...
            var data = RES.getRes("startBtn_json");
            var txtr = RES.getRes("startBtn_png");
            var mcFactory: egret.MovieClipDataFactory = new egret.MovieClipDataFactory(data,txtr);
            this.startBtn = new egret.MovieClip(mcFactory.generateMovieClipData("startBtn"));
            this.addChild(this.startBtn);
            var mc = this.startBtn;
            mc.scaleX = 0.5;
            mc.scaleY = 0.5;
            mc.play(-1);
            mc.touchEnabled = true;
            mc.x = this.width / 2 - mc.width / 4 - 30;
            mc.y = this.height / 2 - mc.height / 2;
            mc.addEventListener(egret.TouchEvent.TOUCH_TAP,this.StartBtnOnClick,this);
        }
 
    }
    
    private StartBtnOnClick(){
        this.removeEventListener(egret.TouchEvent.TOUCH_TAP,this.StartBtnOnClick,this);
